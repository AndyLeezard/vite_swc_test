...
import JitsiConnection, {
  JitsiConferenceOptions,
} from "@solyd/lib-jitsi-meet/dist/esm/JitsiConnection"
...
export class JitsiMeet {
  ...
  private options: JitsiMeetOptions = { connectionOptions: { hosts: {} } }

  private connectionEventListeners:
    | Map<JitsiConnectionEvents, Function>
    | undefined
  private conferenceEventListeners:
    | Map<JitsiConferenceEvents, Function>
    | undefined

  public static APP_ID = "vpaas-magic-cookie-813e574d1406486ba58b50b5a5e5fea1"
  public static KEY_ID =
    "vpaas-magic-cookie-813e574d1406486ba58b50b5a5e5fea1/568825"

  /**
   * Configuration for the JaaS 8x8 vc service.
   * Settings got from JaaS example project here : https://github.com/jitsi/ljm-jaas-example
   */
  public static CONFIG_JAAS(room: string): JitsiMeetOptions {
    const REGION = "frankfurt" // "default" or "london"
    const REGION_SHARD = "eu-central-1" // "default" or "eu-west-2"
    return {
      ...
      connectionOptions: {
        ...
        hosts: {
          domain: "8x8.vc",
          muc: `conference.${JitsiMeet.APP_ID}.8x8.vc`,
          focus: "focus.8x8.vc",
        },
        ...
        serviceUrl: `wss://${REGION}.8x8.vc/${JitsiMeet.APP_ID}/xmpp-websocket?room=${room}`,
        websocketKeepAliveUrl: `https://${REGION}.8x8.vc/${JitsiMeet.APP_ID}/_unlock?room=${room}`,
        enableP2P: true,
        p2p: {
          enabled: true,
          preferH264: true,
        ...
        confID: `https://8x8.vc/${JitsiMeet.APP_ID}/${room}`,
        siteID: JitsiMeet.APP_ID,
        applicationName: "escape web light",

        // Misc
        deploymentInfo: { userRegion: REGION_SHARD },
  ...
  public static get CONFIG_MEET_JIT_SI(): JitsiMeetOptions {
    return {
      ...
      connectionOptions: {
        ...
        get serviceUrl() {
          return (
            "https://meet.jit.si/http-bind?room=" + this.roomName?.toLowerCase()
          )
        },
  ...
  public async connect(
    jwtoken: any,
    listeners?: Map<JitsiConnectionEvents, Function>
  ): Promise<string> {
    return new Promise<string>((resolve, reject) => {
      ...
      console.log(`JITSI connexion using LJM version ${JitsiMeetJS.version}!`)
      ...
      this.connection.addEventListener(
        ...
        (e: JitsiConnectionErrors) => reject(new Error(e))
      )
      this.connection.addEventListener(
        JitsiConnectionEvents.WRONG_STATE,
        (e: JitsiConnectionErrors) => reject(new Error(e))
      )
      this.connection.addEventListener(
  ...
  public async joinConference(
    name: string,
    listeners?: Map<JitsiConferenceEvents, Function>
  ): Promise<boolean> {
    name = name.toLowerCase()
    // TODO: Cancel other already running joinConference()s
    if (!this.connection) {
      console.info(
        "joinConference has been called without an existing connection, doing nothing"
      )
      return true
    }
    ...
    if (name && this.options?.connectionOptions?.roomName) {
      console.warn(
        `Room name overridden by options.connectionOptions.roomName (${this.options.connectionOptions.roomName} instead of ${name}). You should only set one. `
      )
    ...
    return new Promise<boolean>(async (resolve, reject) => {
      ...
      if (this.conference?.isJoined()) {
        ...
        if (this.conference?.getName() === name.toLowerCase()) {
          console.info(
            `That's the same conference as requested (${name}), doing nothing`
          )
          resolve(false)
          return
        } else {
      ...
      this.conference?.on(
        ...
        (e: JitsiConferenceErrors) => reject(new Error(e))
      )
  ...
  public async leaveConference(): Promise<boolean> {
    ...
    return new Promise<boolean>(async (resolve) => {
      ...
      this.connection.xmpp.eventEmitter.removeAllListeners(
  ...
  public async createLocalTracks(
    ...
    let tracksOptions = options ||
      this.options.trackOptions || { devices: ["audio", "video"] }

    return new Promise<void>(async (resolve) => {
      console.debug("Creating local tracks", this.conference)
      let tracks: JitsiLocalTrack[] = (await JitsiMeetJS.createLocalTracks(
        tracksOptions
      )) as unknown as JitsiLocalTrack[] // createLocalTracks can also return JitsiConferenceErrors
